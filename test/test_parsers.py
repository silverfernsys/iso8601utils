...
class TestParsers(unittest.TestCase):
    ...
    def test_interval(self, mock_datetime):
        ...
        self.assertEqual(interval('P6Y5M'),
                         (now - timedelta(days=(365 * 6)) - monthdelta(5), None))
        ...
        self.assertEqual(interval('1999-12-31T16:00:00.000Z'),
            (datetime(year=1999, month=12, day=31, hour=16), None))
        self.assertEqual(interval('2016-08-01T23:10:59.111Z'),
        ...
        self.assertEqual(interval('P6Y5M/P9D'),
                         (now - timedelta(days=365 * 6) - monthdelta(5),
        ...
        self.assertEqual(interval('2016-08-01T23:10:59.111Z/2016-08-08T00:13:23.001Z'),
    ...
    def test_duration(self):
        self.assertEqual(duration('P3Y6M4DT12H30M5S'),
                         (timedelta(days=3 * 365 + 4, hours=12,
                         minutes=30, seconds=5), monthdelta(6)))
        ...
        self.assertEqual(duration('P6M1DT'),
                         (timedelta(days=1), monthdelta(6)))
        self.assertEqual(duration('P6W'),
                         (timedelta(weeks=6), monthdelta(0)))
        self.assertEqual(duration('P5M3DT5S'),
                         (timedelta(days=3, seconds=5), monthdelta(5)))
        self.assertEqual(duration('P3Y4DT12H5S'),
                         (timedelta(days=365 * 3 + 4, hours=12, seconds=5), monthdelta(0)))
        self.assertEqual(duration('P3Y4DT12H30M0.5005S'),
                         (timedelta(days=365 * 3 + 4, hours=12, minutes=30,
                          milliseconds=500, microseconds=500), monthdelta(0)))
        self.assertEqual(duration('PT.5005S'),
                         (timedelta(milliseconds=500, microseconds=500), monthdelta(0)))
        self.assertEqual(duration('P6Yasdf'), (None, None))
        self.assertEqual(duration('7432891'), (None, None))
        self.assertEqual(duration('asdf'), (None, None))
        self.assertEqual(duration('23P7DT5H'), (None, None))
        self.assertEqual(duration(''), (None, None))
